...
if [[ "$PANE_COUNT" -ge 3 ]]; then
    ...
    if [[ "$EXP_TAG" == "exp2" ]]; then
        ...
        tmux send-keys -t $SESSION:0.2 "sudo tcpdump -i dpUdpTunServer tcp -w $EXP_DIR/${EXP_TAG}-$SUFFIX-tcp-red-80s-10M-server.pcap" C-m
    else
        # exp1: tcpdump 檔名不包含 det/5g
        tmux send-keys -t $SESSION:0.2 "sudo tcpdump -i dpUdpTunServer tcp -w $EXP_DIR/${EXP_TAG}-tcp-red-ul-80s-10M-server.pcap" C-m
    fi
