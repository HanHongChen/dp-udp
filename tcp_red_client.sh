...
if [[ "$PANE_COUNT" -ge 2 ]]; then
    ...
    tmux send-keys -t $SESSION:0.1 "iperf3 -c 192.180.20.10 -B 192.180.10.10 -t 80 -b " C-m
...
if [[ "$PANE_COUNT" -ge 3 ]]; then
    ...
    if [[ "$EXP_TAG" == "exp2" ]]; then
        ...
        tmux send-keys -t $SESSION:0.2 "tcpdump -i dpUdpTunClient tcp -w $EXP_DIR/${EXP_TAG}-$SUFFIX-tcp-red-80s-10M-client.pcap" C-m
    else
        # exp1: tcpdump 檔名不包含 det/5g
        tmux send-keys -t $SESSION:0.2 "tcpdump -i dpUdpTunClient tcp -w $EXP_DIR/${EXP_TAG}-tcp-red-ul-80s-10M-client.pcap" C-m
    fi
    tmux select-pane -t $SESSION:0.1
fi
